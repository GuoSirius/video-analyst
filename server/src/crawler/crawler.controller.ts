...
import { ScopeConfig, extractLinks, inScope } from './site-crawler.util'
...
export class CrawlerController {
  ...
  startTask(@Param('id') id: string) {
    ...
    if (task.status === 'paused') {
      ...
      this.runCrawl(id, task.payload, resumeState)
    } else if (task.status === 'pending') {
      this.runCrawl(id, task.payload)
    } else {
  ...
  private async processCrawlTask(taskId: string, payload: CrawlPayload, resumeState?: any) {
    const {
      url, itemSelector, nextPageSelector,
    ...
    try {
      ...
      while (true) {
        ...
        const ext = await this.extractAndInsertItems(html, currentUrl, taskId, payload, timeoutMs, errorMode)
        totalItems += ext.items
        skippedItems += ext.skipped

        if (urlPattern && isUnlimited && ext.items === 0) {
  ...
  /**
   * 解析 HTML、提取条目（含可选详情页）、识别媒体并写入 crawl_items。
   * single / list / site 三种模式共用，避免逻辑重复。
   * @returns 本次插入的条目数与跳过的条目数（详情页失败）
   */
  private async extractAndInsertItems(
    html: string,
    currentUrl: string,
    taskId: string,
    ...
    const {
      rules, itemSelector, detailRules,
      titleField, detailLinkField, mediaUrlField, urlTransforms = [],
    } = payload
    ...
    const items = this.crawler.parseHtml(
      html, rules, itemSelector ? itemSelector : undefined, currentUrl,
    )
  ...
  /**
   * 根据任务模式分发到对应的执行引擎。
   * site 模式走整站递归 BFS；single / list 走原有线性分页引擎。
   */
  private runCrawl(taskId: string, payload: CrawlPayload, resumeState?: any) {
    if (payload.mode === 'site') {
      this.processSiteTask(taskId, payload, resumeState)
    } else {
      this.processCrawlTask(taskId, payload, resumeState)
    }
  }

  // ════════════════════════════════════════════════════════════════
  // 整站递归爬取引擎（BFS frontier）
  // ════════════════════════════════════════════════════════════════

  private async processSiteTask(taskId: string, payload: CrawlPayload, resumeState?: any) {
    const {
      url,
      errorMode = 'standard',
      sameDomain = true,
    ...
    let entryHost = ''
    try {
      entryHost = new URL(url).host
    } catch {
      this.queue.updateTaskError(taskId, `Invalid entry URL: ${url}`)
      return
    }

    const scope: ScopeConfig = { sameDomain, allowPaths, denyPaths, includeRegex, excludeRegex }

    let pagesVisited = resumeState?.pagesVisited || 0
    let linksDiscovered = resumeState?.linksDiscovered || 0
    let itemsFound = resumeState?.itemsFound || 0
    let errors = resumeState?.errors || 0
    let skipped = resumeState?.skipped || 0

    try {
      this.queue.updateTaskStatus(taskId, 'running')
      this.queue.updateTaskProgress(taskId, 0)

      // 初始化 frontier（仅在非续跑时）。续跑时 pending 队列已持久化，直接继续。
      if (!resumeState) {
        this.clearFrontier(taskId)
        this.enqueueFrontier(taskId, url, 0, null)
      }

      while (true) {
        const cur = this.queue.getTask(taskId)
        if (!cur || cur.status === 'cancelled') return
        if (cur.status === 'paused') {
          this.queue.updateTaskResult(taskId, {
            pagesVisited, linksDiscovered, itemsFound, errors, skipped,
            pagesPending: this.countFrontier(taskId, 'pending'),
          })
          return
        }
        ...
        const next = this.popNextPending(taskId)
        if (!next) break
        ...
        try {
          ...
          this.markFrontier(taskId, currentUrl, 'failed', e?.message || String(e))
          errors++
          continue
        }

        // 1) 链接发现 + 范围过滤 + 去重入队
        const links = extractLinks(html, linkSelector, currentUrl)
        for (const link of links) {
          if (!inScope(link, scope, entryHost)) continue
          if (maxDepth > 0 && depth + 1 > maxDepth) continue
          if (this.enqueueFrontier(taskId, link, depth + 1, currentUrl)) {
            linksDiscovered++
          }
        }

        // 2) 页面提取（命中 itemSelector 走列表式，否则走单页式），复用既有逻辑
        const ext = await this.extractAndInsertItems(html, currentUrl, taskId, payload, timeoutMs, errorMode)
        itemsFound += ext.items
        skipped += ext.skipped

        this.markFrontier(taskId, currentUrl, 'visited')
        this.updateSiteProgress(taskId, pagesVisited, maxPages, this.countFrontier(taskId, 'pending'))

        if (delayMs > 0) await new Promise(r => setTimeout(r, delayMs))
      }

      this.queue.updateTaskResult(taskId, {
        pagesVisited, linksDiscovered, itemsFound, errors, skipped, pagesPending: 0,
      })
    } catch (e: any) {
      this.queue.updateTaskError(taskId, e?.message || String(e))
    }
  }

  /** 整站进度：有 maxPages 按已访问/上限；无限时按 已访问/(已访问+待访问) 估算，最多 99% */
  private updateSiteProgress(taskId: string, pagesVisited: number, maxPages: number, pending: number) {
    let pct: number
    if (maxPages > 0) {
      pct = Math.min(Math.round((pagesVisited / maxPages) * 100), 99)
    } else if (pagesVisited + pending > 0) {
      pct = Math.min(Math.round((pagesVisited / (pagesVisited + pending)) * 100), 99)
    } else {
      pct = 0
    }
  ...
  private clearFrontier(taskId: string) {
    this.db.db.prepare('DELETE FROM crawl_frontier WHERE task_id = ?').run(taskId)
  }

  private countFrontier(taskId: string, status: string): number {
    const row = this.db.db.prepare('SELECT COUNT(*) AS c FROM crawl_frontier WHERE task_id = ? AND status = ?').get(taskId, status) as any
    return row?.c || 0
  }

  /** 入队一个待访问 URL（已归一化）。INSERT OR IGNORE 实现去重；返回是否为新入队 */
  private enqueueFrontier(taskId: string, url: string, depth: number, parentUrl: string | null): boolean {
    try {
      const r = this.db.db.prepare(
        `INSERT OR IGNORE INTO crawl_frontier (id, task_id, url, depth, status, parent_url, discovered_at) VALUES (?, ?, ?, ?, 'pending', ?, datetime('now'))`,
      ).run(uuid(), taskId, url, depth, parentUrl)
      return r.changes > 0
    } catch {
      return false
    }
  ...
  /** 取下一个待访问 URL（BFS：先按 depth 升序，再按 id）。取出即标记为 visited，避免单 worker 重复处理 */
  private popNextPending(taskId: string): { url: string; depth: number } | null {
    const row = this.db.db.prepare(
      `SELECT url, depth FROM crawl_frontier WHERE task_id = ? AND status = 'pending' ORDER BY depth ASC, id ASC LIMIT 1`,
    ).get(taskId) as any
    if (!row) return null
    this.db.db.prepare(
      `UPDATE crawl_frontier SET status = 'visited', visited_at = datetime('now') WHERE task_id = ? AND url = ?`,
    ).run(taskId, row.url)
    return { url: row.url, depth: row.depth }
  }

  private markFrontier(taskId: string, url: string, status: string, error?: string) {
    this.db.db.prepare(
      `UPDATE crawl_frontier SET status = ?, error = ?, visited_at = datetime('now') WHERE task_id = ? AND url = ?`,
    ).run(status, error ?? null, taskId, url)
  }
