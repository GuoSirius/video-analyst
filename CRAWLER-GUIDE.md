...
| **整站爬取** | 从入口递归发现站内链接、按范围过滤并去重，逐页提取 | 跨多页的数组 | 整站采集、站点地图式抓取 |

```
单页采集: URL → fetch → parseHtml(rules) → 1 个 item
列表采集: URL → fetch → parseHtml(rules, itemSelector) → N 个 items
整站爬取: 入口URL → 入队 → 取下一个pending → fetch → 发现链接(范围过滤+去重)入队 → 提取(命中itemSelector走列表式/否则单页式) → 循环直到队列空或达上限
```
...
从站点入口 URL 出发，**递归、逐步**地爬取整站：自动发现页面中的链接 → 按范围规则过滤 → 去重后进入待访问队列（frontier）→ 逐页执行提取。后端复用既有的 `parseHtml`、详情页提取、媒体识别与入库逻辑，无需重写。

### 核心机制

```
入口URL ──→ 入队(pending, depth=0)
loop:
  取下一个 pending（BFS：先按 depth 升序）
  检查暂停/取消/上限（maxPages / maxDepth）
  fetch HTML
  1) 链接发现：extractLinks(linkSelector) → 绝对化 → 按范围规则过滤 → 去重入队(depth+1)
  2) 页面提取：命中 itemSelector → 列表式提取多条；否则单页式提取一条
  mark visited → 更新进度 → SSE 推送
  直到 frontier 无 pending 或达上限
```

- **去重**：`crawl_frontier` 表以 `(task_id, url)` 唯一，`INSERT OR IGNORE` 保证同一 URL 只入队一次。
- **续跑**：暂停时 pending 队列持久化保留，继续时直接从上次位置恢复；`result` 中保存 `pagesVisited / linksDiscovered / itemsFound` 等计数器。
- **复用**：详情页提取（`detailRules` + `detailLinkField`）、媒体识别、字段指定、容错级别与单页/列表模式完全一致。

### 配置项

| 配置 | 默认 | 说明 |
|------|------|------|
| 同域名限制 (`sameDomain`) | `true` | 仅爬与入口同主域的链接（含子域），**默认防爬到外站** |
| 最大递归深度 (`maxDepth`) | 0 | 0=无限；>0 时只抓取入口起 N 层内的链接 |
| 最大访问页数 (`maxPages`) | 0 | 0=无限；>0 时访问满 N 页停止 |
| 请求间隔 (`delayMs`) | 200 | 两次请求间的礼貌间隔（ms），降低被封风险 |
| 链接发现选择器 (`linkSelector`) | `a[href]` | 用于发现站内链接的 CSS 选择器，可限定到导航区 |
| 列表项选择器 (`itemSelector`) | 可选 | 填写则仅命中此选择器的页面按「列表式」提取多条；留空则每个被访问页面按「单页式」提取一条 |
| 路径白名单 (`allowPaths`) | [] | 仅允许的路径前缀，如 `/docs`、`/blog` |
| 路径黑名单 (`denyPaths`) | [] | 排除的路径前缀，如 `/login`、`/cart`、`/admin` |
| URL 正则白名单 (`includeRegex`) | [] | 任一匹配才保留 |
| URL 正则黑名单 (`excludeRegex`) | [] | 任一匹配即排除（优先级高于白名单） |

### 范围过滤顺序

```
同域(可关闭) → 路径白名单 → 路径黑名单 → 正则白名单 → 正则黑名单
```

过滤在 `server/src/crawler/site-crawler.util.ts` 的 `inScope()` 中实现，纯函数便于单测。

### 适用建议

- 中小站点（数千页内）顺序 BFS 流畅运行；超大站点建议设置 `maxPages` / `maxDepth` 与较大的 `delayMs` 控制规模与风控。
- 外站链接默认被 `sameDomain` 拦截，不会爬出当前域名。
