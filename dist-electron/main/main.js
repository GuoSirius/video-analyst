import require$$1$2, { app, session, ipcMain, BrowserWindow, nativeImage, Tray, Menu, dialog, shell } from "electron";
import require$$0$1, { resolve as resolve$1, join } from "path";
import require$$0, { existsSync } from "fs";
...
const electronApp = {
  setAppUserModelId(id2) {
    if (platform.isWindows)
      app.setAppUserModelId(is.dev ? process.execPath : id2);
  },
  setAutoLaunch(auto) {
    if (platform.isLinux)
      return false;
...
let tray = null;
const windows$1 = [];
function setTrayWindow(win) {
  windows$1.push(win);
}
function createTray() {
  let iconImage;
  const trayIconPath = resolve$1(__dirname, "../../resources/icons/tray.png");
  const fallbackIconPath = resolve$1(__dirname, "../../resources/icons/icon.png");
  ...
  function updateTrayMenu() {
    const allVisible = windows$1.some((w) => w.isVisible());
    const contextMenu = Menu.buildFromTemplate([
      {
        label: allVisible ? "隐藏所有窗口" : "显示所有窗口",
        click: () => {
          if (allVisible) {
            windows$1.forEach((w) => w.hide());
          } else {
            windows$1.forEach((w) => w.show());
          }
          updateTrayMenu();
        }
      },
...
async function extractAudio(inputPath, outputPath, onProgress) {
...
async function transcribeWithLocalWhisper(audioPath, outputPath, onProgress) {
  const ffmpegPath = getFfmpegPath();
  return new Promise((resolve2, reject) => {
    const args = [
      "-i",
...
function requireSource() {
  ...
  (function(module, exports$1) {
    ...
    let parentDir = "";
    try {
      delete require2.cache[__filename];
...
function requireElectronStore() {
  ...
  const path = require$$0$1;
  const { app: app2, ipcMain: ipcMain2, ipcRenderer, shell: shell2 } = require$$1$2;
...
async function processFile(task, method, llmConfigId, onProgress) {
  try {
...
async function transcribeWithCloudAPI(audioPath, config, onProgress) {
  if (config.provider === "openai") {
    const fs2 = await import("fs/promises");
    const audioData = await fs2.readFile(audioPath);
    ...
    await fs2.writeFile(outputPath, "模拟转文字结果");
    onProgress?.(100);
    return outputPath;
  }
...
function deleteLLMConfig(id2) {
  const configs = getLLMConfigs().filter((c) => c.id !== id2);
  store.set("llmConfigs", configs);
}
function getLLMConfig(id2) {
  return getLLMConfigs().find((c) => c.id === id2);
}
...
function createWindow() {
  ...
  const id2 = mainWindow.id;
  windows.set(id2, mainWindow);
...
function setupWindowIPC() {
  ...
  ipcMain.handle("window-set-always-on-top", (event, onTop, level) => {
    const window = BrowserWindow.fromWebContents(event.sender);
    window?.setAlwaysOnTop(onTop, level || "normal");
  ...
  ipcMain.handle("window-create", (event, options) => {
    ...
    const id2 = win.id;
    windows.set(id2, win);
    win.on("closed", () => windows.delete(id2));
    return { id: win.id };
  });
  ...
  ipcMain.handle("llm:deleteConfig", (event, id2) => {
    deleteLLMConfig(id2);
    return true;
  });
}
