...
// Step 1: 采集模式
const formMode = ref<'single' | 'list' | 'site'>('single')
...
// ── 整站爬取配置（formMode === 'site'）──
const formSameDomain = ref(true)
const formMaxDepth = ref(0)
const formSiteMaxPages = ref(0)
const formDelayMs = ref(200)
const formLinkSelector = ref('a[href]')
const formAllowPaths = ref('')
const formDenyPaths = ref('')
const formIncludeRegex = ref('')
const formExcludeRegex = ref('')
...
/** 将多行/逗号分隔的文本转为字符串数组（修剪空白、去空） */
function splitList(val: string | string[]): string[] {
...
function resetForm() {
  ...
  formSameDomain.value = true
  formMaxDepth.value = 0
  formSiteMaxPages.value = 0
  formDelayMs.value = 200
  formLinkSelector.value = 'a[href]'
  formAllowPaths.value = ''
  formDenyPaths.value = ''
  formIncludeRegex.value = ''
  formExcludeRegex.value = ''
  formRules.value = [
    { name: 'title', selector: 'h1,.title,[class*="title"]', attr: '', regex: '' },
    { name: 'content', selector: '.content,.article-body,[class*="content"]', attr: '', regex: '' },
  ]
...
function openEditDialog(task: any) {
  ...
  // 整站配置
  formSameDomain.value = p.sameDomain ?? true
  formMaxDepth.value = p.maxDepth ?? 0
  formSiteMaxPages.value = p.maxPages ?? 0
  formDelayMs.value = p.delayMs ?? 200
  formLinkSelector.value = p.linkSelector || 'a[href]'
  formAllowPaths.value = Array.isArray(p.allowPaths) ? p.allowPaths.join('\n') : (p.allowPaths || '')
  formDenyPaths.value = Array.isArray(p.denyPaths) ? p.denyPaths.join('\n') : (p.denyPaths || '')
  formIncludeRegex.value = Array.isArray(p.includeRegex) ? p.includeRegex.join('\n') : (p.includeRegex || '')
  formExcludeRegex.value = Array.isArray(p.excludeRegex) ? p.excludeRegex.join('\n') : (p.excludeRegex || '')
...
function setMode(mode: 'single' | 'list' | 'site') {
  formMode.value = mode
  if (mode === 'single') {
...
async function submitForm() {
  ...
  if (formMode.value === 'list') {
    payload.itemSelector = formItemSelector.value || undefined
    payload.paginationMode = formPaginationMode.value
    payload.nextPageSelector = formNextPageSelector.value || undefined
    payload.maxPages = formPaginationMode.value === 'page' ? formMaxPages.value : undefined
    payload.maxItems = formPaginationMode.value === 'count' ? formMaxItems.value : undefined
    payload.loadMoreSelector = formLoadMoreSelector.value || undefined
    payload.urlPattern = formUrlPattern.value || undefined
    payload.pageStart = formUrlPattern.value ? formPageStart.value : undefined
    payload.detailRules = formDetailRules.value.filter(r => r.name).length
      ? formDetailRules.value.filter(r => r.name) : undefined
  } else if (formMode.value === 'site') {
    // 整站模式：itemSelector 可选（命中则按列表式提取，否则按单页式提取每页一条）
    payload.itemSelector = formItemSelector.value || undefined
    payload.detailRules = formDetailRules.value.filter(r => r.name).length
      ? formDetailRules.value.filter(r => r.name) : undefined
    payload.sameDomain = formSameDomain.value
    payload.maxDepth = formMaxDepth.value
    payload.maxPages = formSiteMaxPages.value
    payload.delayMs = formDelayMs.value
    payload.linkSelector = formLinkSelector.value || 'a[href]'
    payload.allowPaths = splitList(formAllowPaths.value)
    payload.denyPaths = splitList(formDenyPaths.value)
    payload.includeRegex = splitList(formIncludeRegex.value)
    payload.excludeRegex = splitList(formExcludeRegex.value)
  }
...
/** 任务采集模式 → 图标与文案（支持整站） */
function taskMode(row: any): 'single' | 'list' | 'site' {
  return row.payload?.mode || (row.payload?.itemSelector ? 'list' : 'single')
}
function taskModeIcon(row: any): string {
  const m = taskMode(row)
  return m === 'single' ? 'fas fa-file-lines' : m === 'list' ? 'fas fa-list' : 'fas fa-globe'
}
function taskModeLabel(row: any): string {
  const m = taskMode(row)
  return m === 'single' ? '单页' : m === 'list' ? '列表' : '整站'
}
      ...
      <el-table v-if="tasks.length" ref="tableRef" :data="tasks" size="small" row-key="id" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="任务名称" min-width="140" show-overflow-tooltip fixed="left">
          <template #default="{ row }">
            <div class="flex items-center gap-2">
              <i :class="[taskModeIcon(row), taskMode(row) === 'site' ? 'text-emerald-400' : taskMode(row) === 'single' ? 'text-blue-400' : 'text-amber-400', 'text-[11px]']" :title="taskModeLabel(row) + '采集'"></i>
        ...
        <el-table-column label="模式" width="80" align="center">
          <template #default="{ row }">
            <span class="text-[11px]" :class="taskMode(row) === 'site' ? 'text-emerald-400' : taskMode(row) === 'single' ? 'text-blue-400' : 'text-amber-400'">
              {{ taskModeLabel(row) }}
            </span>
          </template>
        </el-table-column>
      ...
      <div class="flex flex-col gap-5">
        ...
        <div class="section">
          ...
          <el-radio-group v-model="formMode" size="small" @change="setMode">
            ...
            <el-radio-button value="site">🌐 整站爬取</el-radio-button>
          </el-radio-group>
          <div class="text-[11px] text-gray-500 mt-1.5">
            {{ formMode === 'single' ? '抓取单个页面内容，如新闻详情页、文章页，结果为一个结构化对象。' : formMode === 'list' ? '抓取列表页面中的所有条目，如产品列表、文章列表，结果为一个数组。' : '从站点入口递归爬取整站：自动发现链接、按范围规则过滤、去重，并对命中规则的页面执行提取。' }}
          </div>
        </div>
        ...
        <div v-if="formMode === 'site'" class="section">
          ...
          <div class="flex items-center gap-4 mb-3">
            ...
            <el-tooltip content="默认开启：仅抓取与入口同主域的链接，避免爬到外部站点。关闭后将抓取发现的任意域名链接（慎用，可能触发风控）。" placement="top">
              <i class="fas fa-circle-question text-gray-600 cursor-help text-[12px]"></i>
            </el-tooltip>
          </div>

          <div class="flex items-center gap-3 mb-3">
            <span class="text-xs text-gray-400 w-28 flex-shrink-0">最大递归深度</span>
            <el-input-number v-model="formMaxDepth" :min="0" :max="50" size="small" />
            <span class="text-[11px] text-gray-500">0 = 无限</span>
          </div>

          <div class="flex items-center gap-3 mb-3">
            <span class="text-xs text-gray-400 w-28 flex-shrink-0">最大访问页数</span>
            <el-input-number v-model="formSiteMaxPages" :min="0" :max="99999" size="small" />
            <span class="text-[11px] text-gray-500">0 = 无限</span>
          </div>

          <div class="flex items-center gap-3 mb-3">
            <span class="text-xs text-gray-400 w-28 flex-shrink-0">请求间隔(ms)</span>
            <el-input-number v-model="formDelayMs" :min="0" :max="60000" :step="50" size="small" />
            <span class="text-[11px] text-gray-500">两次请求之间的礼貌间隔，防封</span>
          </div>

          <div class="mb-3">
            <div class="text-xs text-gray-400 mb-1">
              链接发现选择器
              <el-tooltip content="用于发现站内链接的 CSS 选择器，默认 a[href]。可限定到导航区，如 nav a[href]、.menu a[href]。" placement="top">
                <i class="fas fa-circle-question text-gray-600 cursor-help text-[11px] ml-1"></i>
              </el-tooltip>
            </div>
            <el-input v-model="formLinkSelector" placeholder="a[href]" size="small" />
          </div>

          <div class="mb-3">
            <div class="text-xs text-gray-400 mb-1">
              列表项选择器 <span class="text-gray-600">（可选）</span>
              <el-tooltip content="若填写，则仅当页面匹配此选择器时按「列表式」提取多条；留空则每个被访问页面按「单页式」提取一条。复用下方提取规则。" placement="top">
                <i class="fas fa-circle-question text-gray-600 cursor-help text-[11px] ml-1"></i>
              </el-tooltip>
            </div>
            <el-input v-model="formItemSelector" placeholder=".article-item, li.post（留空=每页提取一条）" size="small" />
          </div>

          <div class="grid grid-cols-2 gap-3 mb-1">
            <div>
              <div class="text-xs text-gray-400 mb-1">路径白名单 <span class="text-gray-600">（每行/逗号一项）</span></div>
              <el-input v-model="formAllowPaths" type="textarea" :rows="2" placeholder="/docs&#10;/blog" size="small" />
            </div>
            <div>
              <div class="text-xs text-gray-400 mb-1">路径黑名单</div>
              <el-input v-model="formDenyPaths" type="textarea" :rows="2" placeholder="/login&#10;/cart&#10;/admin" size="small" />
            </div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <div class="text-xs text-gray-400 mb-1">URL 正则白名单</div>
              <el-input v-model="formIncludeRegex" type="textarea" :rows="2" placeholder="\.html$&#10;/product/" size="small" />
            </div>
            <div>
              <div class="text-xs text-gray-400 mb-1">URL 正则黑名单</div>
              <el-input v-model="formExcludeRegex" type="textarea" :rows="2" placeholder="\.css$&#10;\?.*utm_" size="small" />
            </div>
          </div>
          <div class="text-[11px] text-gray-500 mt-2">
            💡 路径规则按前缀匹配（如 <code class="text-amber-400 bg-amber-500/10 px-1 rounded">/docs</code>）；正则黑名单优先级高于白名单。白名单/黑名单留空表示不限制。
          </div>
        </div>
