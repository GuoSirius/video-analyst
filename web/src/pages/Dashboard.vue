...
async function refresh() {
  const [ct, tt, wt, at, items] = await Promise.all([
    crawlerAPI.getTasks(), transcoderAPI.getTasks(), whisperAPI.getTasks(), aiAPI.getTasks(), crawlerAPI.getItems(),
  ])
...
<template>
  <div class="p-6 max-w-[1200px]">
    ...
    <p class="text-sm text-gray-500 mb-6">音视频采集 · 转码 · 识别 · 分析一站式处理</p>

    <div class="grid grid-cols-4 gap-4 mb-6">
      <div v-for="m in modules" :key="m.key" @click="router.push(m.path)"
        class="bg-gray-800/60 rounded-xl border border-gray-700/50 p-5 cursor-pointer hover:border-gray-600/60 transition-colors">
    ...
    <div class="grid grid-cols-5 gap-5">
      <div class="col-span-3 bg-gray-800/60 rounded-xl border border-gray-700/50 p-5">
        ...
        <div v-else class="space-y-1">
          <div v-for="t in recentTasks" :key="t.id" class="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-gray-900/40">
            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px]"
              :class="t.status === 'completed' ? 'bg-emerald-500/15 text-emerald-300' : t.status === 'running' ? 'bg-blue-500/15 text-blue-300' : t.status === 'failed' ? 'bg-red-500/15 text-red-300' : 'bg-yellow-500/15 text-yellow-300'">
              {{ t.status === 'completed' ? '完成' : t.status === 'running' ? '进行' : t.status === 'failed' ? '失败' : '等待' }}</span>
      ...
      <div class="col-span-2 bg-gray-800/60 rounded-xl border border-gray-700/50 p-5">
