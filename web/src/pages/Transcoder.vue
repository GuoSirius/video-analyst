...
async function retryTask(id:string){await transcoderAPI.retryTask(id);ElMessage.success('已重新加入队列');refreshTasks()}
      ...
      <el-table v-if="tasks.length" :data="tasks" size="small">
        <el-table-column label="任务" min-width="200">
          <template #default="{row}">
            <div class="text-xs text-gray-300 truncate max-w-[260px]">{{ row.payload?.file?.split(/[\\/]/).pop() || row.id.slice(0,12)+'...' }}</div>
